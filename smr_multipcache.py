...
band_count = (diskset_size - PCACHE_UNIT) // BAND_SIZE
...
def cleanPCache(time,devno):
    ...
    for band in sorted(dirty_band):
        starting_blkno = band * BAND_SIZE + ((band * BAND_SIZE) // (BAND_SIZE * band_count) + 1) * PCACHE_UNIT
...
def handleRead(time, devno, blkno, blkcount):
    ...
    blkno += (blkno // (BAND_SIZE * band_count) + 1) * PCACHE_UNIT
    while blkcount > 0:
...
def printConfiguration():
    print("------------Configuration------------")
    print("Persistent cache size: " + str(TOTAL_PCACHE))
    print("Band size: " + str(BAND_SIZE))
    print("Bands that follow a persistent cache unit: " + str(band_count))
